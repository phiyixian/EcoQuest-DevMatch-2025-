...
/*Frontend tip:
    ...
    2. User nonid to input treasuryDonation contract address
            Hardcode or configure the treasuryDonation contract address in your frontend code 
            (for example, in a config file or as a constant).*/

/* To refine:
    - offset amount shld be fixed (value passed from extension/frontend)
    - treasuryDonation contract address (spender) shld not be input by user*/

/* MockUSDC contract is to create usdc token for deployer
    and user can approve transaction (neccessary before offset)*/

/* treasuryDonation contract is to let user transfer usdc to klimadao (offset)*/
...
/// @title treasuryDonation - Donate USDC to KlimaDAO
contract treasuryDonation {
